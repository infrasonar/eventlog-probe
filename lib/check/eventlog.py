...
class CheckEventlog(Check):
    ...
    async def run(asset: Asset, local_config: dict, config: dict) -> dict:
        custom = set(config.get('eventCodes', []))
        include_security = config.get('securityEvents', True)
        source = config.get('matchSource', '')
        event_code, security = [], []
        if include_security:
            sec_ec = set(SECURITY)
        ...
        query = Query(f"""
            ...
            WHERE TimeWritten > "{start.strftime('%Y%m%d%H%M%S.000000-000')}"
            AND
        ...
        for (ct, last, items, codes) in (
            ...
            for ec in codes:
                ...
                if ec in last:
                    ...
                    item['LastEventType'] = EVENT_TYPE.get(
                        last[ec]['EventType'])
